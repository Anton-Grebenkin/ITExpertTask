...
using ITExpertTask.Exceptions;
using ITExpertTask.Util;
using Microsoft.EntityFrameworkCore;
using System.Linq.Expressions;
using System.Runtime.CompilerServices;

namespace ITExpertTask.Extensions
{
    public static class DbSetExtension
    {
        public static IQueryable<T> ApplyFilter<T>(this DbSet<T> dbSet, IEnumerable<DataFilter> filters) where T : class
        {
            ...
            {
                var property = properties.FirstOrDefault(p => p.Name.ToLower() == filter.PropertyName.ToLower());
                ...
                if (!JsonElementHelper.TryConvert(filter.Value, member.Type, out var value))
                    throw new FilterException($"Failed to convert filter {filter.PropertyName} value to type {member.Type.Name}");
